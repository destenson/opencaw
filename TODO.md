...
- Index git commit history as recallable memory: the indexer could ingest the repo's commit log (and possibly per-commit diffs) as stubs, so relevant commits are surfaced and injected the same way file content is — making the repository's history part of the model's memory, not just its working tree. A mid-task question like "when did we switch X to Y" or "what was the rationale for this change" would then resolve to the commit(s) that made it, the way a signature question resolves to a file. Distinct from the git-aware-workspace item above (that reacts to current state changes; this indexes the past). Open questions before building: what granularity (commit metadata only vs full diff vs commit message + touched paths), how a commit stub is ranked against file stubs for a query, how to avoid drowning the index in churn on high-activity repos, and the same general-purpose-library boundary as the git-aware item (a generic "history source" hook, not git hardcoded in the library). Future work, not v0.1.
