...
- [ ] **Fix B3: session-log live-insertion path uses in-memory HNSW only**: `build_index.rs::should_skip` now excludes `.caw*` paths, but the CLI inserts active session logs into the live HNSW index via `SessionFile::load_previous`, and those entries reach the persistent SQLite store on flush. This means sessions started within minutes of a prior session retrieve that session's log as authoritative context (observed in QA 0008: session started 75 seconds after prior session ended, prior session's improvised answer retrieved as grounded documentation). The fix is to add session log entries to an in-memory-only vector index that is not persisted to the store and not written to disk.
- [ ] **Minimum-content filter for chunk indexing**: Chunks whose extractable text is below ~20 meaningful tokens (punctuation, keywords, and whitespace excluded) should not be indexed, or should fall back to a description derived from the parent stub's summary. Examples causing noise: `./crates/caw-core/Cargo.toml` chunks that contain only `[package]` (3 tokens), and trailing code chunks like `| ".git"` from path-filter arrays. These consume retrieval slots and show up in search-candidates lists with no useful signal. Observed across QA loops 0002, 0003, 0005, 0008 — still unfixed.
