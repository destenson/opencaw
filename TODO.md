...
- [ ] **Summary-content-length filter**: The minimum-content filter (token_estimate < 10) filters by raw file size, not by the length of the actual injectable content (summary + outline). Crate-level Cargo.toml stubs have token_estimate=50–337 but summary="[package]" (3 real tokens of signal). Add a secondary filter that skips stubs where summary content after trimming is below a minimum length (e.g., 15 chars). Observed in QA loop 0009 (recommendation 4).
...
- [ ] **Summarize prior session history before in-memory HNSW indexing (B7)**: `collect_previous_stubs` loads prior session logs as full text into the in-memory retrieval pool. Model responses (400–450 tokens each) outcompete workspace stubs for retrieval slots because they contain every relevant concept in polished prose. Instead, prior session content should be summarized before indexing: user queries kept verbatim, model responses condensed to 1–3 key conclusions. The detailed response text can still appear in the session history fragment, but the indexed embedding should be of the summary, not the full response. Observed in QA loop 0009 (recommendation 1; documented as B7).
...
- [ ] **Cap eviction note accumulation per stub**: After N evictions (suggested: 2), collapse additional eviction notes into a single "Evicted N times, most recently relevance=X" summary instead of appending another full-text note. By QA loop 0009 turn 3, `probe_recall.rs` had 3 eviction notes consuming 286+ tokens for repetitive mechanical data. Observed in QA loop 0009 (recommendation 5).
