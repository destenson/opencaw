...
The streams differ by surface — `caw-bench` separates report from logs, `caw-cli` does not:

- **Report (stdout).** `caw-bench` and `caw-bench-graph-eval` print only their report to stdout (tracing is sent to stderr, so `graph-eval.sh ... > report.txt` captures a clean report with no log interleaving). Do **not** use `2>&1 | grep` on these — that re-merges the log stream you just separated.
- **Logs.** Diagnostic logging goes through `tracing`, quiet by default (deps at `warn`, app at `info`; per-call spam such as the candle embedder's per-batch timing is at `debug` and therefore off). Turn it up only when you need it: `RUST_LOG=debug ...` for everything, or scoped, e.g. `RUST_LOG=caw_index=debug ...` for embedder timing, `RUST_LOG=caw_orchestrator=debug ...` for recall decisions. An explicit `RUST_LOG` is honored verbatim. **Where the logs land depends on the binary:**
  - *`caw-bench` / `caw-bench-graph-eval`* write tracing to **stderr** (the report-stream separation above).
  - *`caw-cli`* writes tracing to **stdout**, on the same stream as the model answer — its `tracing_subscriber::fmt()` init has no `.with_writer(...)`, so the default writer is stdout. The short status lines it prints itself (`[intent] …`, `[workspace: N fragments, ~T tokens]`, the `Enter queries:` prompt) go to **stderr**. Consequence: `2>log` captures only those status lines, not tracing — to capture caw-cli's tracing you must redirect stdout (`> log`) or merge (`2>&1`). This is intentional for caw-cli; do not add `.with_writer(stderr)` to "fix" it.
...
- **A collapsed recall@k can be spurious `stale` flags, not a real regression.** Stubs marked `stale=1` are excluded from `all_embeddings()` and the read-only eval path, so they vanish from retrieval. A since-fixed bug marked stubs stale on a transient content-read miss; the flags persist in the SQLite index. If a known-good eval suddenly drops (e.g. sysdoc recall@10 0.84→0.37), check `sqlite3 <index> "select count(*) from stubs where stale=1;"` before suspecting code — repair without rebuilding via `sqlite3 <index> "UPDATE stubs SET stale=0 WHERE stale=1;"`. Eval scripts open the store read-only so they won't re-poison it.
...
- **`scripts/extract-sessions.py`** — flatten Claude Code session transcripts (`~/.claude/projects/<slug>/*.jsonl`) into per-session prose docs for indexing as a recall corpus. With no out-dir it reports the staging dir it created; must not be under .gitignored paths.
