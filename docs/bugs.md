...
Related side effect noted while fixing (not addressed here): under a wrong corpus-root, `get_content` marks every probed/queried stub `stale=1` in the SQLite index via `mark_path_stale`. The proxy has no reingest worker, so those rows stay stale and a subsequent *correct* run would see a degraded index until rebuilt. Marking-stale-on-missing-body is the right behavior for the CLI live index (file genuinely changed/deleted) but is inappropriate for the proxy's read-only prebuilt index. Candidate follow-up: give the proxy a read-only store mode that skips `mark_path_stale`.
