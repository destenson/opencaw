...
## B22. `extract_field` includes assistant response text in user field of session history — FIXED
...
## B24. Proxy corpus-root mismatch fails semi-silently — returns HTTP 200 unaugmented (medium) — FIXED
...
Found while end-to-end testing the caw-dev skill (2026-05-31).

Fixed (2026-06-01) in `caw-server/src/lib.rs`: both signals from the proposed fix were added. (1) A startup corpus-root probe runs right after the index loads and before the BM25 body pass — it samples `CORPUS_ROOT_PROBE_SAMPLE` (8) stub bodies and, if none are readable, emits one aggregate `error!` naming the corpus-root and an example missing path ("…every request will forward UNAUGMENTED"); a partial miss emits a `warn!`. The stub path is read via `get_stub` *before* `get_content`, because a missing body makes `get_content` call `mark_path_stale`, after which `get_stub` itself returns `StaleStub` and the example path would be lost. (2) `retrieve_fragments` now counts content-fetch misses and emits one `error!` per request when every candidate had an unreadable body, so a mid-run corpus problem is visible without summing per-stub warns.

Related side effect (also fixed, 2026-06-01): under a wrong corpus-root, `get_content` marked every probed/queried stub `stale=1` in the SQLite index via `mark_path_stale`. The proxy has no reingest worker, so those rows stayed stale and a subsequent *correct* run saw a degraded index (injecting nothing) until rebuilt. Marking-stale-on-missing-body is right for the CLI live index (file genuinely changed/deleted) but wrong for the proxy's read-only prebuilt index. Fix: `SqliteStubStore::with_read_only(bool)` — in read-only mode `get_content` still returns `StaleStub` so the caller skips the stub, but never calls `mark_path_stale` or enqueues reingestion. `caw-server::build_state` opens the store with `with_read_only(true)`. Verified end-to-end: a wrong-root run followed by a correct-root run on the same index still injects (previously it injected zero because the wrong run had marked every row stale).
