...
- **Cross-item concurrency is opt-in, never the primary lever, and never forbidden.** Empirically it does not speed up a sweep on the current single-Ollama-server two-GPU setup: same-model requests serialize (4-concurrent scaled ~linearly, not flat), the `-worker2` alias does not load as a second resident instance, and one 9B decode leaves only ~18% spare GPU headroom (see `docs/findings.md`, "Negative result: cross-item concurrency"). The runner's serial `for item { for mode }` loop stays the default, but the design must keep a `--concurrency N` capability (default 1) available for a setup where the server genuinely parallelizes or a second instance truly lands on the second GPU — it is disallowed to *hardcode* serial-only. When concurrency > 1, per-item phase timing (gen/judge/other) is invalid (shared judge counter + wall-time contention) and must be gated to the serial path.
