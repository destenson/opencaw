...
**Status:** authoritative reference for implementation decisions. Read `docs/origin.md` for the founding design; read this document before making any significant implementation choice.
...
**Default workspace token budget: 12000 (was 2000 on the server).** `caw-server`'s CLI defaulted `max_workspace_tokens` to 2000 — the same deliberately-tight value the bench's `RunnerConfig::default` uses to force the eviction machinery to fire — while the orchestrator library default (`DynamicRecallConfig::default`) was 12000. A real agent talking to the proxy got the study budget. Measured (2026-06-15, deterministic `code-agent` sweep over this repo, groq answer): at 2000, recall-on lost to recall-off on two exact-fact lookups (`ca_005`, `ca_012`) because eviction reduced the gold body to a stub before the answer turn; at 12000 both flip to parity and the set goes flat (answer_score 0W/12T/1L). 12000 is not a tuned value — just large enough that normal recall is not clamped out; the right value depends on the upstream context window, so hosts should override per `--max-workspace-tokens`. Extracted as `caw_orchestrator::DEFAULT_MAX_WORKSPACE_TOKENS`; the server mirrors it as a literal rather than taking a dependency on the orchestrator crate (the proxy is the single-shot RAG path and does not otherwise use it).
...
- **Reconcile near-miss classifier key names by prefix-stripped token equality (2026-06-16).** A small intent classifier (`granite4:micro`) emitted `is_explanation` where the schema field is `wants_explanation`; serde's `#[serde(flatten)] extra` swallowed the unrecognized key, so the signal was lost (empty `QueryIntent()`) *and* the stray key leaked into the answer prompt as a synthesized `Additional context: is explanation.` guidance line. `QueryIntent::from_classifier_response` now normalizes each leftover `extra` bool key — strip the verb prefix the schema varies on (`is_`/`wants_`/`needs_`/…) and a trailing `_request` — and folds it onto the matching field. This is a deliberate, documented heuristic (the project default is to avoid heuristics): the classifier is a small model that does not reliably reproduce exact field spellings, and recovering the signal is worth the added matching. Match is by **equality of the normalized token, not substring**, on purpose — substring matching would wrongly absorb legitimate model-authored hints such as `needs_git_status` (which contains "status") into `is_status_request`. Separately, the `guidance_lines` layer that turns intent flags and `extra` keys into canned prose is flagged in TODO.md as a prototype leftover to redesign; this decision only covers the parser-side reconciliation.
...
1. Check `docs/origin.md` §11 for settled answers to architectural questions.
