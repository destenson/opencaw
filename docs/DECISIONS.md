...
- **A small answer model is the primary thesis testbed, not just a cost choice (2026-06-14).** Context-as-workspace claims that curating the workspace so only relevant content is resident lets the model do more with less. That value is *most visible on a small model*: a large model compensates for a cluttered or gold-buried workspace with parametric knowledge and noise-tolerant reasoning, so its recall-on − recall-off delta is muted; a small model has little to fall back on, so the same curation quality shows up as a larger delta. The testable prediction: the recall-on − recall-off delta is larger on a small model (e.g. groq `llama-3.1-8b-instant`) than on a large one, and well-curated small-model output approaches larger-model output. This makes the small model the more *sensitive* instrument for measuring curation, and reframes the "pin the target model" caveat above — the thesis target is deliberately small. It also raises the stakes on the recall regression (recall-on burying gold, BUGS "Retrieval"): that defect hurts a small model most, so fixing it is both the step-2 priority and the cleanest way to demonstrate the thesis. See `design.md` for the thesis itself.
