...
Next: the lever is now **prompt/cooperation design, not retrieval.** Three candidate moves (none picked yet — the small model is the sensitive defect-catcher and the "never short-circuit the model / never hardcode responses" guardrails apply, so this needs a decision, not a default): (1) reframe `response_requirement` so it does not actively invite hedging when the cited source contains the answer (risk: over-assertion/hallucination when the source doesn't); (2) tone down the cooperation instructions' probe/think-out-loud hedging bias; (3) confirm with a stronger answer model whether the same quoted source gets asserted — isolates "small-model over-caution" from "prompt-induced hedging" (costs groq tokens). Adjacent, still gated on the now-trustworthy instrument: (a) expand the thin `code-agent` QA set (14 items) for gauge power; (b) judge calibration on near-correct signatures — moot for `ca_005` until the model actually answers instead of hedging.
