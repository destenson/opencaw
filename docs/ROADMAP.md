...
2. **Enabling the loop is not yet a net answer-quality win at 2k.** Cooperation fixed `ca_005` but regressed `ca_006`/`ca_008`; `mean_answer_score` 0.429 → 0.377. The loop is now doing something, and that something introduces new failures on items basic-RAG handled. The work is in *what the loop does once it fires* (eviction policy, probe usefulness, refinement), not in PD itself. The `caw-bench-coop` run also crashed at item 4 on a degenerate-output abort — the orchestrator aborts rather than retries there (a harness-robustness bug, BUGS candidate).
...
✓ Done (2026-06-18): progressive disclosure **implemented** (`f11146e`) and **tested**. It is mechanically sound but inert under the bench's default `Auto`+non-reasoning-adapter config; with the answer model run cooperative it fires (22 upgrades, `ca_005` fixed), but net answer_score is still slightly negative at 2k because the multi-pass loop regresses items basic-RAG handled (`ca_006`, `ca_008`).

Next: **decide the cooperation default for the bench answer model and then tune what the loop does once it fires.** The immediate question is whether `cooperation_mode: Cooperative` for the recall-on path (currently an uncommitted working-tree toggle in `runner.rs`) should be kept as the bench default so the loop is always exercising — without it, every recall-on/off sweep is measuring basic RAG in both modes and PD can never be evaluated. Assuming yes: the real work is then step 3's second caution — the loop, now that it fires, breaks `ca_006`/`ca_008`. Diagnose those two regressions (likely eviction or probe-driven load-order effects) before expanding the QA set or tuning ranking. The 12k budget should be re-measured with cooperation on too — the 2026-06-15 "12k fixes ca_005" result was with the loop inert, so it does not actually tell us what the loop does at a realistic budget.
