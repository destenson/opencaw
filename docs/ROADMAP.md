...
Next: **the real open loop-behavior question is `ca_005` on at a realistic budget.** The 2026-06-15 "12k fixes `ca_005`" result was with the loop inert (cooperation off), so it does not represent what the loop does at 12k. With the loop firing at 12k, `ca_005` on is a **hedge** (0.0): the gold body is resident but the model answers "not fully specified… we can infer" instead of stating the signature. The budget window where PD helps is narrower than the inert runs suggested. So the genuine loop-quality work is: diagnose why the cooperative loop hedges instead of answering once the gold body is resident — distinct from the scoring artifacts just removed. Two adjacent items, both now gated on the trustworthy instrument: (a) expand the thin `code-agent` QA set (14 items) so the gauge has power; (b) judge calibration on near-correct signatures — the judge scored `ca_005` off 0.0 for a signature differing from the reference only by the param name `query` vs `query_embedding`, harsh on a substantively-correct answer (an instrument-trust question, not a recall question).
