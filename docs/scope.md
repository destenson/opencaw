...
- [ ] **Middleware / proxy that runs the orchestrator** (any OpenAI-compatible 3rd-party client): an `--orchestrate` mode in caw-server where the upstream model becomes the orchestrator's `ModelAdapter` (via `OpenAiCompatibleAdapter`) and the server drives the multi-pass recall loop, instead of the single-shot retrieve→inject→forward path. Buffered response for v0 (one SSE chunk when `stream:true`); the existing single-shot path stays the default so v0 doesn't regress. Active v0.1 work. Targets clients that accept a custom OpenAI base URL (Codex, Cursor, aider); Anthropic-protocol clients are served by the passthrough route below.
- [x] **Anthropic Messages passthrough** (`/v1/messages` route in caw-server): accepts the Anthropic Messages request shape so the official `anthropic` SDK (and Anthropic-protocol clients generally) can point at the proxy unmodified. Same single-shot retrieve→inject→forward path as the OpenAI route; the only protocol-specific handling is reading/injecting block-array `content` and the upstream path. Response is streamed back verbatim, so the upstream must itself speak the Anthropic protocol (e.g. `https://api.anthropic.com`). This is passthrough only — it does not run the orchestrator. See `docs/DECISIONS.md`.
...
- **Cross-protocol translation proxy** (dropped, not deferred): a proxy where the *client* and *upstream* speak different protocols — e.g. an Anthropic-protocol client in front of an OpenAI-only upstream — requiring bidirectional request + response + SSE translation. This is **out of scope for any version as a passthrough proxy** (confirmed 2026-06-15): translating a streamed response is a fundamentally larger job than mutating a request, and it buys nothing the same-protocol passthrough routes don't already cover. The OpenAI-compatible orchestrator proxy ("Should have") and the Anthropic Messages passthrough route (done, "Should have") each keep client and upstream on one protocol; neither translates between them.
- **Engine plugins** (vLLM, Ollama native, etc.): Deep integration for mid-token recall. Requires the framework to stabilize first.
- **Server** (caw-server): Functional retrieval-augmentation proxy speaking both the OpenAI (`/v1/chat/completions`) and Anthropic (`/v1/messages`) request shapes. The single-shot retrieve→inject→forward path retrieves context, augments the last user message, and forwards with streaming passthrough; it stays the default. The orchestrator-backed `--orchestrate` mode (multi-pass recall) is active v0.1 work — see "Should have". Uses Candle + CUDA for embeddings.
